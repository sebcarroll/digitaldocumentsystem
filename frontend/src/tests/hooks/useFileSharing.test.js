...
jest.mock('../../services/permissions_and_sharing_service');
...
describe('useFileSharing', () => {
  ...
  beforeEach(() => {
    ...
    permissionsApi.getCurrentUserRole.mockResolvedValue({ role: 'owner', id: 'user1' });
    permissionsApi.getPeopleWithAccess.mockResolvedValue({
      peopleWithAccess: [{ id: 'user1', role: 'viewer' }],
      currentUserRole: 'owner',
      currentUserId: 'user1',
      generalAccess: 'Restricted'
    });
    permissionsApi.shareFile.mockResolvedValue({});
    permissionsApi.updatePermission.mockResolvedValue({});
    permissionsApi.removePermission.mockResolvedValue({});
    permissionsApi.updateGeneralAccess.mockResolvedValue({});
  });
  ...
  test('should update email and trigger user search on handleEmailChange', async () => {
    usersApi.searchUsers.mockResolvedValue({ users: [{ id: 'user1', email: 'user1@example.com' }] });
    ...
    expect(usersApi.searchUsers).toHaveBeenCalledWith('user');
  ...
  test('should share files with pending emails on handleShareWithPendingEmails', async () => {
    ...
    expect(permissionsApi.shareFile).toHaveBeenCalledTimes(4); // 2 files * 2 emails
  ...
  test('should update access level for a user on handleAccessLevelChange', async () => {
    ...
    expect(permissionsApi.updatePermission).toHaveBeenCalledTimes(2); // 2 files
    expect(permissionsApi.updatePermission).toHaveBeenCalledWith('file1', 'user1', 'editor');
    expect(permissionsApi.updatePermission).toHaveBeenCalledWith('file2', 'user1', 'editor');
  });
  ...
  test('should remove access for a user on handleRemoveAccess', async () => {
    ...
    expect(permissionsApi.removePermission).toHaveBeenCalledTimes(2); // 2 files
    expect(permissionsApi.removePermission).toHaveBeenCalledWith('file1', 'user1');
    expect(permissionsApi.removePermission).toHaveBeenCalledWith('file2', 'user1');
  });
  ...
  test('should update general access on handleGeneralAccessChange', async () => {
    ...
    expect(permissionsApi.updateGeneralAccess).toHaveBeenCalledTimes(2); // 2 files
    expect(permissionsApi.updateGeneralAccess).toHaveBeenCalledWith('file1', 'Anyone with the link', 'viewer');
    expect(permissionsApi.updateGeneralAccess).toHaveBeenCalledWith('file2', 'Anyone with the link', 'viewer');
