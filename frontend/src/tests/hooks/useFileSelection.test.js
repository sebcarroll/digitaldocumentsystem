...
import { moveFiles, deleteFiles, copyFiles, openDriveFile, renameFile } from '../../services/api.js';
...
describe('useFileSelection', () => {
  ...
  it('should handle file movement', async () => {
    ...
    expect(moveFiles).toHaveBeenCalledWith(fileIds, newFolderId);
  ...
  it('should handle file deletion', async () => {
    ...
    expect(deleteFiles).toHaveBeenCalledWith([file.id]);
    expect(getDriveFiles).toHaveBeenCalledWith(currentFolder.id);
  ...
  it('should handle copying file link', async () => {
    ...
    openDriveFile.mockResolvedValue(response);
    ...
    await act(async () => {
      await result.current.handleCopyLink(file);
    });
  
    expect(openDriveFile).toHaveBeenCalledWith(file.id);
  ...
  it('should handle errors when copying file link', async () => {
    const file = { id: 'fileId123' };
    const error = new Error('Failed to open file');
    openDriveFile.mockRejectedValue(error);
    ...
    expect(openDriveFile).toHaveBeenCalledWith(file.id);
    expect(setError).toHaveBeenCalledWith(`Failed to copy link: ${error.message}`);
  });
  
  it('should not proceed if no file is provided to handleCopyLink', async () => {
    const { result } = renderHook(() =>
      useFileSelection(getDriveFiles, currentFolder, setError)
    );
  
    await act(async () => {
      await result.current.handleCopyLink(null);
    });
  ...
  it('should handle file renaming', async () => {
    ...
    renameFile.mockResolvedValue();  // Mock the API call
    ...
    expect(renameFile).toHaveBeenCalledWith(file.id, newName);
  ...
  it('should handle making a copy of files', async () => {
    ...
    expect(copyFiles).toHaveBeenCalledWith([file.id]);
    expect(getDriveFiles).toHaveBeenCalledWith(currentFolder.id);
    expect(result.current.selectedFiles).toHaveLength(0);
  });
