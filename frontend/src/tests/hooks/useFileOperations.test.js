...
import { createFolder, uploadFile, uploadFolder, createDoc, createSheet } from '../../services/api.js'
...
describe('useFileOperations', () => {
  ...
  it('should handle folder creation', async () => {
    ...
    expect(createFolder).toHaveBeenCalledWith(currentFolder.id, folderName);
  ...
  it('should handle file upload', async () => {
    ...
    expect(uploadFile).toHaveBeenCalledWith(currentFolder.id, file);
    expect(getDriveFiles).toHaveBeenCalledWith(currentFolder.id);
  });
  ...
  it('should handle folder upload', async () => {
    ...
    uploadFolder.mockResolvedValue({});
    ...
    expect(uploadFolder).toHaveBeenCalledWith(currentFolder.id, files);
    expect(getDriveFiles).toHaveBeenCalledWith(currentFolder.id);
  });
  ...
  it('should handle Google Doc creation', async () => {
    ...
    createDoc.mockResolvedValue(response);
    ...
    expect(createDoc).toHaveBeenCalledWith(currentFolder.id);
    expect(getDriveFiles).toHaveBeenCalledWith(currentFolder.id);
  ...
  it('should handle Google Sheet creation', async () => {
    ...
    createSheet.mockResolvedValue(response);
    ...
    expect(createSheet).toHaveBeenCalledWith(currentFolder.id);
    expect(getDriveFiles).toHaveBeenCalledWith(currentFolder.id);
    expect(window.open).toHaveBeenCalledWith(response.webViewLink, '_blank', 'noopener,noreferrer');
  });
